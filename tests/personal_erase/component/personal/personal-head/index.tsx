...
class PersonalTitle extends Component<any, any> {
  logInAndRegList      = null;
  hasLogInAndRegIcon          = false;
  scrollEmitter      = null;
  ...
  getWidth = () => {
    const width         = CommonUtils.getWindowSize('personal').screenWidth;
    const height         = CommonUtils.getWindowSize('personal').screenHeight;
    return { width, height };
  };
  ...
  private initVariable(vproList     , newMergeStyle     ) {
  ...
  private getMarginLeft(showText     , marginLeft        , showIcon     ) {
    if (showText) {
      marginLeft = -5;
    } else if (showIcon) {
      marginLeft = -3;
  ...
  private getMergeStyle(newMergeStyle     , isMember     , theme) {
