...
class MyOrder extends PureComponent<any, any> {
  ...
  linkFinished     ;
  apkGotourlMy     ;
  hmiSpanId     ; // begin埋点返回的数据，里面有spanId
  constructor(props     ) {
    super(props);
    this.state = {
  ...
  isWeb = () => {
    const width         = CommonUtils.getWindowSize('personal').screenWidth;
  ...
  async componentDidMount() {
    ...
    this.eventListener = LoginUtils.listenLoginStatus((isLogin         ) => {
      if (isLogin === false && this.linkFinished === false) {
        this.linkFinished = true;
        this.apkGotourlMy = '';
      }
    });
    this.cancelLoginListener = DeviceEventEmitter.addListener('NativeCallAction', (event     ) => {
  ...
  _onclick = async (key        ) => {
    ...
    if (isLogin) {
      ...
      if (this.isApp()) {
        this.eventListener = LoginUtils.listenLoginStatus((login         ) => {
  ...
  private renderReturnRefundFor(commonParams     , _styles     , styleContent     ) {
  ...
  private renderToBeEvaluated(commonParams     , _styles     , styleContent     ) {
    const {
      iconWidth,
      isDarkTheme,
      normalColor,
  ...
  private renderToBeReceived(commonParams     , _styles     , styleContent     ) {
    const {
      iconWidth,
      isDarkTheme,
      normalColor,
  ...
  private renderPedingPayment(commonParams     , _styles     , styleContent     ) {
    const {
      iconWidth,
      isDarkTheme,
      normalColor,
  ...
  private renderRecyclingOrderForBigTxt(commonParams     , _styles     , styleContent     ) {
  ...
  private renderReturnRefundForBigTxt(commonParams     , _styles     , styleContent     ) {
    const {
      bigFontBoxStyle,
      isLogin,
      bigFontMarkStyle,
  ...
  private renderToBeEvaluatedForBigTxt(commonParams     , _styles     , styleContent     ) {
    const {
      bigFontBoxStyle,
      isLogin,
      count,
  ...
  private renderToBeReceivedForBigTxt(commonParams     , _styles     , styleContent     ) {
    const {
      bigFontBoxStyle,
      isLogin,
      unreceiptOrderCount,
  ...
  private renderPedingPaymentForBigTxt(commonParams     , _styles     , styleContent     ) {
    const {
      bigFontBoxStyle,
      isLogin,
      unpaidOrderCount,
  ...
  private renderAllOrder(_styles     , isDarkTheme         ) {
    return (
      <TouchableOpacity
        activeOpacity={1}
        onPress={() => {
  ...
  private getStyleContent(_styles     ) {
