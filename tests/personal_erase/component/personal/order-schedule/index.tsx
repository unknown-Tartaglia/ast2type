...
class OrderSchedule extends PureComponent<any, any> {
  ...
  isWeb = () => {
    const width         = CommonUtils.getWindowSize(PAGE_TYPE.personal).screenWidth;
  ...
  private getRemoveClippedSubviews(isRemoveClippedSubviews         )      {
  ...
  renderSwiperDot = (imgList     , swiperItemWidth        ) => {
    return (
      <SwiperDots
  ...
  render() {
    ...
    const imgListViwApp = this.imgListWap.map((val     , index        ) => {
      const imgObj = {
