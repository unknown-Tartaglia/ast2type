...
const CONSUME_YEAR_TYPE           = ['1', '2']; // 1"：未成年账号 “2”表示儿童

const lottieViewHeight         = 96;
...
class Personal extends React.Component<any, any> {
  ...
  scrollViewKey         = 1;
  refreshControl                      ;
  upgradeGiftInterval      = 0;
  lastGiftPopupTime      = null;
  lastCalendarPopupTime      = null;
  giftPoint      = 0;
   ...
   */
  enableAdvancedFeature         = Number(this.props.enableAdvancedFeature);
  calendarAuthLinstener      = null;
  sonRef     ;
  animatedEvent     ;
  navOpacityAnimated     ;
  isPersonal      = true;
  ...
  deviceRes     ;
  userId        ;
  dialogEventLisenter      = null;
  isGroupFlag          = false;
  isDisplayScenePurchaseNavBottom          = false; // 是否展示场景购的底部导航
  scenePurchaseNavBottomData      = {}; // 场景购底部导航的数据
  pageListener           = new Listener('Personal');
  hmiSpanIdFirstScreen        ; // firstScreen begin埋点返回的数据，里面有spanId
  hmiSpanIdFinalScreen        ; // finalScreen begin埋点返回的数据，里面有spanId
  getRecommendFlagTimer     ;
  isCaculated          = false;
  consumeYear         = '1';
  remainDays         = 30;
  recyclerListOneByTwo          = true; // 商品卡1*2样式是否使用recyclerList加载
  eventOffsetY     ;
  constructor(props     ) {
    super(props);
    this.state = {
    ...
    if (this.props.abTest) {
      let ab      = JSON.parse(this.props.abTest);
  ...
  getSwitchConfig() {
    const couponConfig                      = configStore.couponUseConfig;
  ...
  private handleListerDialog()      {
    return DeviceEventEmitter.addListener('NativeCallAction', (event     ) => {
  ...
  private handlePopStrategies(event     , activityCode     , strategyContent     , strategies     ) {
  ...
  // 调用apk弹窗接口弹窗
  popStrategyDialog(activityCode        ) {
  ...
  async componentDidMount() {
    ...
    if (PlatformUtils.isApp()) {
      RnBridge.invokeVmallNative('common', 'getCartAndMessageNmb', {}).then((result     ) => {
  ...
  private handleMessageNum(event     ) {
  ...
  private getAppLoginAndGroupInfo() {
    LoginUtils.getAppLoginAndGroupInfo().then((result     ) => {
      if (result) {
  ...
  private async getLoginData(isLogin     , avaImg     , userName     , userAccount     ) {
    if (isLogin) {
  ...
  private handleReport(mineData     , pageId     , event     ) {
  ...
  private reportLeavePersonPage(reportPageId     ) {
  ...
  private reportLoadPersonPage(reportPageId     ) {
    EventTracking.eventTracingReport({
  ...
  private personNetworkRefresh(event     , isLogin     , avaImg     , userName     , userAccount     ) {
  ...
  // 消息浮层打开状态下，监听状态
  private handleNotifiactionStatus(event     ) {
  ...
  // 页面载入的埋点
  eventTracingReportByLoad = (isRefresh?         ) => {
    this.isLeave = true;
    EventTracking.eventTracingReport({
      actionCode: '110000000',
      actionName: '加载个人中心组件化页面',
  ...
  _onRefresh() {
    ...
    if (PlatformUtils.isIOS()) {
      LoginUtils.getLoginStatus().then((result     ) => {
        this.setState({
          isJsLogin: result,
        });
        if (result) {
  ...
  _queryUserInfo = (avgImg?     , userName?     , userAccount?     ) => {
  ...
  private updateReportPageId(data     , isRefresh         ) {
    this.setState(
      {
  ...
  private getAndroidPageInfoCatch(isRefresh         , e     ) {
  ...
  private handleUserInfo(user     , avgImg     , userName     , userAccount     ) {
  ...
  private updateUserInfo(user     , avgImg     , userName     , userAccount     ) {
  ...
  getVproLogo = () => {
    ...
    HttpService.getTemplate(holderParams)
      .then((res) => {
        if (res?.success) {
          const tmpMap      = res.templateMapping;
   ...
   * @private
   */
  _renderApp = (styles     , theme     , groupStyleChangesProps     , headStyle     ) => {
   ...
   * @private
   */
  _renderAppTop = (groupStyleChangesProps     , headStyle     ) => {
  ...
  renderLoginComponent = (convertResult     ) => {
  ...
  getBackTopOffsetHeight = (convertResult     ) => {
    let heigth = 0;
    // 场景购距离底部的高度
  ...
  // 通知弹窗是否展示
  isShowNotifyDialogFunc = async (isShowNotify = false, result     ) => {
  ...
  // 弹窗展示信息获取
  getNotifyInfo = () => {
    RnBridge.invokeVmallNative('common', 'getNotificationSwitch').then(async (result     ) => {
      if (result) {
  ...
  // 检查native返回的开关状态
  getNotificationSwitchStatus = (params     ) => {
    return (
      params.system === '0' ||
      params.logistics === '0' ||
      params.service === '0' ||
  ...
  private handleLogin(event     , param     ) {
  ...
  private handleRenderApp(jsonCards     ) {
  ...
  private getIsMember(disPlayApk     ) {
    return (
  ...
  private showDynamicComponent(pageInfos     ) {
    return (
      <DynamicComponent
        // @ts-expect-error 忽略TS检查
  ...
  private showMarginTop(isMemberShow     ) {
    return (
      <View
        style={{
  ...
  private showFixedButton(styles     , pageInfos     ) {
    return (
