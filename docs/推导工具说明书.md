...
```bash
mkdir -p /tmp/ast2type-ts-demo-src
cp tests/basic/1.ts /tmp/ast2type-ts-demo-src/index.ts

npm run migration -- migrate-ts /tmp/ast2type-ts-demo-src \
...
```bash
npm run migration -- check /tmp/ast2type-demo-out \
  --contract uniform \
  --declarations /tmp/ast2type-demo-declarations
```
...
```bash
npm run migration -- repair /tmp/ast2type-demo-out \
  --out /tmp/ast2type-demo-fixed \
  --strategy rules \
  --contract uniform \
  --rule-rounds 5 \
  --max-diagnostics 10
...
需要启用 Agent 修复时，将 `--strategy rules` 改为 `rules+agent`，并补充
`--agent-provider`、`--agent-model` 和对应 API 环境变量。
